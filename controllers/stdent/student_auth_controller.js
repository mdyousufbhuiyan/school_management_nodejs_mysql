...
exports.loginController = async (req, res) => {
  ...
  const sql = `SELECT * FROM ${constants.STUDENT_TABLE} WHERE user_id = ?`;
  mysqldb.query(sql, [req.body.username], (err, rows, fields) => {
    if (err instanceof Error) {
     ...
     ( bcrypt.compareSync(req.body.password, rows[0].password) ||
      ...
      const sql = `UPDATE ${constants.STUDENT_TABLE} SET token = ? WHERE id = ?`;
      mysqldb.query(sql, [token, rows[0]["id"]], (err, result) => {
        ...
        if (err instanceof Error) {
          ...
          const sql = `SELECT id,token,user_id,user_type,profile,email,nid,first_name,first_name_bn,last_name,last_name_bn,gender,dob,phone,is_active,created_at,updated_at FROM ${constants.STUDENT_TABLE} WHERE user_id = ?`;
          mysqldb.query(sql, [req.body.username], (err, rows, fields) => {
...
exports.changePasswordController = async (req, res) => {
  console.log(`user name ${req.body.student_id}`);
  const sql = `SELECT * FROM ${constants.STUDENT_TABLE} WHERE id = ?`;
  mysqldb.query(sql, [req.body.student_id], (err, rows, fields) => {
    if (err instanceof Error) {
      console.log(err);
      return res.status(statusCode.STATUS_BAD_REQUEST).json({
        message: staticMessage.FAILED(req.headers.language),
     ...
     ( bcrypt.compareSync(req.body.old_password, rows[0].password) ||
      ...
      //console.log(`token.........${token}`);
      const sql = `UPDATE ${constants.STUDENT_TABLE} SET password = ? WHERE id = ?`;
      mysqldb.query(sql, [hash, req.body.student_id], (err, result) => {
        console.log(`........update response.........${result}`);
