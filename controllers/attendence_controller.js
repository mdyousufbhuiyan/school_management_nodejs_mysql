...
exports.getAllAttendence = (req, res) => {
  ...
  ${getRelationalQuery(req.headers.language)} WHERE ${conditions} ORDER BY ${tObj}.attendance_date DESC`;
  else
    sql = `SELECT 
  ${this.selectedFields(tObj)},
  ${getRelationalQuery(req.headers.language)} ORDER BY ${tObj}.attendance_date DESC`;
...
exports.getAttendanceWithStudentsDetails = (req, res) => {
  ...
  if (queryList.length > 0) {
    sql = `SELECT 
  ${this.selectedFields(tObj)},
  ${getRelationalQueryWithStudentDetails} WHERE ${conditions} GROUP BY ${tObj}.id ORDER BY ${tObj}.attendance_date DESC`;
  } else {
    sql = `SELECT 
  ${this.selectedFields(tObj)},
  ${getRelationalQueryWithStudentDetails} ORDER BY ${tObj}.attendance_date DESC`;
  }
  db.query(sql, queryList, (err, rows, fields) => {
    if (err instanceof Error) {
      console.log(err);
...
exports.getAttendanceForPerticularMonth = (req, res) => {
  ...
  if (queryList.length > 0) {
    sql = `SELECT 
  ${this.selectedFields(tObj)} FROM ${tName} as ${tObj}  WHERE ${conditions} GROUP BY ${tObj}.id ORDER BY ${tObj}.attendance_date DESC`;
  } else {
    sql = `SELECT 
  ${this.selectedFields(tObj)} FROM ${tName} as ${tObj} ORDER BY ${tObj}.attendance_date DESC`;
  }
  db.query(sql, queryList, (err, rows, fields) => {
    if (err instanceof Error) {
      console.log(err);
      ...
      if (rows.length > 0) {
        ...
        db.query(query, values, (err, result) => {
          ...
          if (result.affectedRows === 0) {
            return res.status(statusCode.STATUS_BAD_REQUEST).json({ message: "Item not found" });
          }
...
JSON_OBJECT('id', s.id,'name', ${language==constants.LANGUAGE_BN?'s.name_bn':'s.name'},'subject_code',s.subject_code) AS subject_info,
JSON_OBJECT('id', t.id,'first_name',${language==constants.LANGUAGE_BN?'t.first_name_bn':'t.first_name'} ,'middle_name',${language==constants.LANGUAGE_BN?'t.middle_name_bn':'t.middle_name'},'last_name', ${language==constants.LANGUAGE_BN?'t.last_name_bn':'t.last_name'}) AS teacher_info
FROM ${tName} as ${tObj}
...
var getRelationalQueryWithStudentDetails = `
...
CONCAT('[',GROUP_CONCAT(JSON_OBJECT('id', st.id,'first_name',st.first_name,'last_name',st.last_name,'user_type',st.user_type,'user_id',st.user_id,
    'roll_no',st.roll_no) ORDER BY st.roll_no ASC SEPARATOR ','),']') AS student_info
FROM ${constants.ATTENDANCE_TABLE_NAME} as ${tObj}
LEFT JOIN ${constants.CLASS_ROOM_TABLE_NAME} as cr ON ${tObj}.class_room_id = cr.id
LEFT JOIN ${constants.CLASS_NAME_TABLE} as c ON cr.class_id = c.id
LEFT JOIN ${constants.GROUP_NAME_TABLE_NAME} as g ON cr.group_id = g.id
LEFT JOIN ${constants.SUBJECT_NAME_TABLE} as s ON ${tObj}.subject_id = s.id
LEFT JOIN ${constants.TEACHER_TABLE} as t ON ${tObj}.teacher_id = t.id
LEFT JOIN ${constants.ACADEMIC_YEAR_TABLE_NAME} as a ON cr.academic_year_id = a.id
LEFT JOIN ${constants.STUDENT_TABLE} as st ON JSON_CONTAINS(${tObj}.student_ids, JSON_ARRAY(st.id))
`;
