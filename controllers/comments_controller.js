...
var tName = constants.COMMENTS_COLLECTION_NAME;
var tObj = "com";
...
exports.getAllComments = (req, res) => {
  ...
  if (req.query.student_id != null) {
   ...
   queryList.push(limit);
      queryList.push(offset);
  var sql;
  console.log(`...conditions..${conditions}.....queryList........${queryList}`);
  if (queryList.length > 0) {
    sql = `SELECT 
  ${this.selectedFields(tObj)}, ${getRelationalQuery(
    req.headers.language
  )} WHERE ${conditions} ORDER BY created_at DESC LIMIT ? OFFSET ?`;
  } else {
    sql = `SELECT 
  ${this.selectedFields(tObj)},${getRelationalQuery(
    req.headers.language
...
},'profile',t.profile) AS teacher_info
FROM ${tName} as ${tObj}
